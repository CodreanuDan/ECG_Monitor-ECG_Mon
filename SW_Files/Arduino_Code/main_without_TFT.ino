...
/* BPM variables */
#define RRINT_ALPHA 0.15f
#define BPM_ALPHA 0.15f

#define QRS_THRESHOLD 0.10f
#define QRS_ALPHA 0.05f

volatile int r_val = 0;
volatile int r_max_val = 0;
...
float convert_raw_to_voltage(int value)
{
	float conv_voltage = (value * ADC_VREF) / ADC_MAX_VAL;
	return conv_voltage;
}
...
void ECG_Proc_MainFunction()
{
	...
	ECG_Proc_GetRawSample();

	int val_to_proc = -1;
	if (!buffer_is_empty(&ecg_buffer))
	{
		val_to_proc = buffer_read(&ecg_buffer);
	}
	else
	{
		return; 
	...
	ECG_Proc_GetMaxPeak(val_to_proc);

	ECG_Proc_GetInstRpeak(val_to_proc);

	ECG_Proc_GetBPM(val_to_proc);

	ECG_Proc_GetQRSWidth(val_to_proc);
...
void ECG_Proc_GetMaxPeak(int raw_val)
{
	/* Detect max peak for R component */
	if (raw_val >= r_max_val)
	{
		r_max_val = raw_val;
	}
}
...
void ECG_Proc_GetInstRpeak(int raw_val)
{
	int peak_threshold = 0.9 * r_max_val;

	static int prev_raw = 0;
	static int prev_d = 0;

	/* Calc. derivata */
	int d = raw_val - prev_raw;
	
	/* Panta pos */
	if (raw_val > peak_threshold)
	{
		/* Deriv din panta pos la panta neg */
		if (prev_d > 0 && d <= 0)
		{
			r_val = raw_val;
		}
...
void ECG_Proc_GetBPM(int raw_val)
{
	...
	if (raw_val > peak_threshold)
	{
		...
		{
			/* Get RR interval as the distance between 2 R peaks */
			unsigned long rr_interval_raw = current_time - last_beat_time;

			/* Write RR_interval in the buffer, get current and average value */
			...
			/* Calculate BPM and apply EMA filter */
			float bpm_raw = 60000 / rr_interval_raw;
			bpm_current = bpm_raw;
...
void ECG_Proc_GetQRSWidth(int raw_val)
{
	...
	int base_threshold = QRS_THRESHOLD * r_max_val;

	if (!in_qrs && raw_val > base_threshold)
	{
		in_qrs = true;
		qrs_counter = 0;
	}
	else if (in_qrs && raw_val < base_threshold)
	{
